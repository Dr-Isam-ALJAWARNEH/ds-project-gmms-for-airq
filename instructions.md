...
generate sufficient amount of new `data samples`. The size of the `generated` data should be either the same as the size of the `original` data that has been used for training for generating the data, or a `representative sample` taken from the original data. This is so to be able to calibrate for accuracy comparisons later on (as discussed [here](#performance-metics)). You should generate sufficient amount of `new samples` to be able to perform subsequent tests. For example, generate 100K (one hundred thousands samples) new samples. Thereafter, you need to test the accuracy and the quality of the generated data based on several query workloads as follows (**N.B.** first, for the NYC Air Quality (AQ) data, the target variable is `pm25`). **N.B.** `you need to perform all those test to the data generated from each of the algorithms that you have designed, comparing their performance`.
# performance metics 
- geo-statistics queries (a.k.a single queries, e.g., `count` and `average`). Example query is `"what is the average pm25 each neighborhood of NYC"`, OR `"what is the pm25 readings count in each neighborhood in NYC"`. 
    - those queries can be tested using the RMSE, and Mean Absolute Percentage Error (MAPE).
- Top-N. Example query: "what are the Top-N neighborhoods in NYC in terms of pm25 values"
    - those queries can be tested using Pearson, [jensen shannon divergence](https://en.wikipedia.org/wiki/Jensen%E2%80%93Shannon_divergence), [Kullback–Leibler divergence](https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence) and other metrics. Attached example notebook on [`NYC taxi mobility data`](https://github.com/Dr-Isam-ALJAWARNEH/ds-project-gmms-for-airq/blob/main/starting-code/stratified-sampling-RMSE-mobility-NYC.ipynb) titled `stratified_sampling_RMSE_mobility_NYC.ipynb` in the `code` folder.
    ...
    > I have attached an example coding for calculating `RMSE`, `MAPE`, [`Spearman correlation coefficient`](https://en.wikipedia.org/wiki/Spearman%27s_rank_correlation_coefficient), read [how it works in geography here](https://geographyfieldwork.com/SpearmansRank.htm#:~:text=Spearman's%20Rank%20correlation%20coefficient%20is%20a%20technique%20which%20can%20be,between%201%20and%20minus%201.), `KL Divergence` and other metrics for the `count`, `avergae` and `Top-N`, respectively. That is to say, how much is the difference in count of `taxi` trips between the original data and the sampled data (new data distribution) for each neighborhood in New York City (NYC). Attached example notebook titled `stratified_sampling_RMSE_mobility_NYC.ipynb` [`here`](https://github.com/Dr-Isam-ALJAWARNEH/ds-project-gmms-for-airq/blob/main/starting-code/stratified-sampling-RMSE-mobility-NYC.ipynb) in the `code` folder.
    > **N.B.** For both query sets, you need to perform spatial join between the point data (i.e., being it sensor air quality reading or taxi mobility in .csv format) and the polygons data (.geosjon file [available online](https://raw.githubusercontent.com/IsamAljawarneh/datasets/master/data/nyc_polygon.geojson)), then you need to perform `groupby` operation on either `neighborhood` or `geohash`, then you compare as per the example attached in [`stratified_sampling_RMSE_mobility_NYC.ipynb`](https://github.com/Dr-Isam-ALJAWARNEH/ds-project-gmms-for-airq/blob/main/starting-code/stratified-sampling-RMSE-mobility-NYC.ipynb) in the `code` folder.
            - Draw x-y graphs, where `x` is the new `sample size` and `y` is the `accuracy` for both types of the queries.
    - One of the workloads also is to perform an [Spatial Approximate Query Processing (SAQP)](https://www.mdpi.com/1424-8220/21/12/4160) relying on a stratified-like sampling which operates on sampling equal fractions from each `geohash` bracket independently (each `geohash` is thus considered a stratum). An example notebook is available [here - spatial_join_sampling_thematic_maps](https://github.com/Dr-Isam-ALJAWARNEH/ds-project-gmms-for-airq/blob/main/starting-code/spatial_join_sampling_thematic_maps.ipynb).
        - so, what you need to do in this case is to take a `representative` sample (i.e., `stratified-like` using geohash values as strata, `strata is the plural, singular is stratum, so each individual geohash is a stratum and all geohash values are strata`), then perform and test the same set of queries as previously (geo-stats, Top-N, DBSCAN `hereafter`), comparing the performance of the `full` `population` newly generated data with the captured `sample` (which is a `miniscule` or `subset` of the data).
...
------------------------------------------------
# dbscan workload
- You need to compare the `dbscan` workload between the `synthetic` new data `generated` using your algorithms and the `original` data. You need to capturte accuracy, most importantly ```silhouette_score```.
...
- sumamry of results you need to capture for both datasets (`mobility` and `air quality`) (**N.B.** `add results to excel sheet and draw figures there`):
    - the baseline are all query workloads by using the original data, need to be compared against the new data generated by each algorithm.
    - for each algorithm, do the following:
        - `RMSE`, `MAPE` for geostatistics (`average` and `count`), baseline agains new data.
        - `KL divergence`, `jensen shannon divergence` for Top-N, baseline agains new data.
        - ```silhouette_score``` for DBSCAN workload, , baseline agains new data.
--------------------------------------------
<!-- Task 5 -->
# [ ] Task 5
